...
const Register = () => {
  ...
  return (
    <LoginForm title='Sign Up' inputs={inputs} submit={{ href: '/', func: submit }} set={set} cancel={{ href: '/login', msg: 'Cancel' }}
      fields={[
        { name: 'email', label: 'Email Address', type: 'text', err: { cond: (inputs.email.length > 20), msg: 'Email is too long.' } },
        { name: 'name', label: 'Name', type: 'text', err: { cond: (inputs.name.length > 20), msg: 'Name is too long.' } },
        { name: 'pass1', label: 'Password', type: 'password', err: { cond: (inputs.pass1.length > 20), msg: 'Password is too long.' } },
